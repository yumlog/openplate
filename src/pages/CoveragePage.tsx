...
export function CoveragePage() {
  ...
  const [cctvOptions, setCctvOptions] = useState<{ value: string; label: string }[]>([]);
  ...
  const currentCctvLabel = cctvOptions.find((c) => c.value === selectedCctv)?.label || "";
  ...
  useEffect(() => {
    ...
    const options = Array.from(svg.querySelectorAll("[inkscape\\:label^='ID ']"))
      .map((el) => {
  ...
  useEffect(() => {
    ...
    const cctvEl = svgContainer?.querySelector(`[inkscape\\:label="ID ${selectedCctv}"]`);
    ...
    const distX = (containerRect.left + containerRect.width / 2) - (cctvRect.left + cctvRect.width / 2);
    const distY = (containerRect.top + containerRect.height / 2) - (cctvRect.top + cctvRect.height / 2);
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      e.preventDefault();
      setZoom((prev) => Math.min(Math.max(prev + (e.deltaY < 0 ? 10 : -10), 50), 500));
    };
  ...
  useEffect(() => {
    ...
    const handleClick = (e: Event) => {
      ...
      target.setAttribute("fill", selectedDirection === 1 ? "#ad46ff" : "#00d5be");
      (selectedDirection === 1 ? setDirection1Selection : setDirection2Selection)((prev) => [
        ...prev,
        id,
      ]);
    };

    const handleEnter = (e: Event) => ((e.currentTarget as SVGElement).style.opacity = "0.8");
    const handleLeave = (e: Event) => ((e.currentTarget as SVGElement).style.opacity = "1");
  ...
  const handleReset = () => {
    ...
    if (svg) {
      selectedIds.forEach((id) => {
        ...
        if (el) el.setAttribute("fill", originalColorsRef.current.get(id) || "");
      });
    }
  ...
  return (
    <div className="flex h-full flex-col gap-5">
      ...
      <div className="flex items-center gap-2">
        ...
        <Button variant="outline" size="icon" onClick={() => handleCctvChange(-1)}>
        ...
        <Button onClick={() => console.log("저장:", { direction1Selection, direction2Selection })}>
        ...
        <Button variant={showUnmatched ? "default" : "outline"} onClick={handleToggleUnmatched}>
      ...
      <div className="flex flex-1 gap-3 overflow-hidden">
        ...
        <div className="flex flex-1 flex-col gap-5 rounded-xl border bg-background p-5">
          <h3 className="text-base font-semibold text-foreground">{currentCctvLabel}</h3>

          <div className="flex flex-col gap-1">
            <span className="text-sm text-foreground">방향1 (상단)</span>
            <div
              className="overflow-hidden rounded-lg border bg-accent"
          ...
          <div className="flex flex-col gap-1">
            ...
            <div
              ...
              style={imageRatio ? { aspectRatio: imageRatio } : undefined}
            >
              <img src={cctvImage} alt="CCTV 방향2" className="w-full -translate-y-1/2" />
            </div>
          </div>
        </div>
        ...
        {/* 우측: 지도-주차칸 선택 */}
        <div className="flex flex-1 flex-col gap-5 rounded-xl border bg-background p-5">
          <h3 className="text-base font-semibold text-foreground">주차칸 선택</h3>
          ...
          <div
            ref={mapContainerRef}
            className="relative flex-1 overflow-hidden rounded-lg border bg-accent"
          ...
          >
            ...
            <div className="absolute right-3 top-3 flex gap-2">
              <Button variant="outline" size="icon" onClick={() => setZoom((p) => Math.min(p + 10, 500))}>
                <ZoomIn className="size-4" />
              </Button>
              <Button variant="outline" size="icon" onClick={() => setZoom((p) => Math.max(p - 10, 50))}>
                <ZoomOut className="size-4" />
              </Button>
              <Button variant="outline" size="icon" onClick={() => { setZoom(100); setPosition({ x: 0, y: 0 }); }}>
                <RotateCcw className="size-4" />
              </Button>
              ...
              <div className="flex flex-wrap gap-1">
                {direction1Selection.length > 0 ? (
                  direction1Selection.map((id) => (
                    <Badge key={id} className="bg-[#ad46ff] tabular-nums">{id}</Badge>
                  ))
                ) : (
              ...
              <div className="flex flex-wrap gap-1">
                {direction2Selection.length > 0 ? (
                  direction2Selection.map((id) => (
                    <Badge key={id} className="bg-[#00d5be] tabular-nums">{id}</Badge>
                  ))
                ) : (
                  <Badge variant="outline">미선택</Badge>
                )}
